...
console.log("ARCH...");
const bible: any = await askJSON(c, P(pr.ARCH, {IDEA: idea, CHAPTERS: String(c.chapters)}), .4, 3, x => {
...
const outline: any = await askJSON(c, P(pr.OUT, {CHAPTERS: "6", WORDS: "14400", ACTS: acts, BIBLE: JSON.stringify(bible)}), .4, 3, validateOutline);
...
console.log("\nHOOK...");
const hook = cleanGeneratedStory(await retryLLM(c, P(pr.HOOK, {WORDS: "260", BIBLE: JSON.stringify(bible), OUTLINE: JSON.stringify(outline)}), {temperature: .7, think: false}, 3, "Hook"));
